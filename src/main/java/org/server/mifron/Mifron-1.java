...
public final class Mifron extends JavaPlugin implements Listener, TabExecutor {
   ...
   private final ShopBlockFeature shopBlockFeature = new ShopBlockFeature(this);
   private final EliteMobFeature eliteMobFeature = new EliteMobFeature(this);
   private final SpecialItemsFeature specialItemsFeature = new SpecialItemsFeature(this);
   private final StoreItemFeature storeItemFeature = new StoreItemFeature(this);
   private final WorldPolicyFeature worldPolicyFeature = new WorldPolicyFeature(this);
   private final EnchantMaxDisplayFeature enchantMaxDisplayFeature = new EnchantMaxDisplayFeature(this);
   ...
   public void onEnable() {
      ...
      this.runStartupStep("register elite mob events", () -> Bukkit.getPluginManager().registerEvents(this.eliteMobFeature, this));
      this.runStartupStep("register special item events", () -> Bukkit.getPluginManager().registerEvents(this.specialItemsFeature, this));
      this.runStartupStep("register store item events", () -> Bukkit.getPluginManager().registerEvents(this.storeItemFeature, this));
      this.runStartupStep("register world policy events", () -> Bukkit.getPluginManager().registerEvents(this.worldPolicyFeature, this));
      this.runStartupStep("register enchant max display events", () -> Bukkit.getPluginManager().registerEvents(this.enchantMaxDisplayFeature, this));
      ...
      this.runStartupStep("apply world policy", this.worldPolicyFeature::apply);
   ...
   ItemStack createOnlineShopProduct(String id) {
      ...
      if ("sell-shelf".equalsIgnoreCase(id) || "sell-shelf-shop".equalsIgnoreCase(id)) {
         return this.createShopBlockItem("SELL_SHELF", Material.OAK_SHELF, "§a販売棚ショップ");
      }
      if ("buy-shelf".equalsIgnoreCase(id) || "buy-shelf-shop".equalsIgnoreCase(id)) {
         return this.createShopBlockItem("BUY_SHELF", Material.SPRUCE_SHELF, "§6買取棚ショップ");
      }
      if ("sell-barrel".equalsIgnoreCase(id) || "sell-barrel-shop".equalsIgnoreCase(id)) {
         return this.createShopBlockItem("SELL_BARREL", Material.BARREL, "§9販売樽ショップ");
      }
      if ("buy-barrel".equalsIgnoreCase(id) || "buy-barrel-shop".equalsIgnoreCase(id)) {
         return this.createShopBlockItem("BUY_BARREL", Material.HOPPER, "§c買取樽ショップ");
      }
   ...
   public boolean canPlaceShopBlock(Player player) {
      return this.shopBlockFeature == null || this.shopBlockFeature.canPlaceShopBlock(player);
   }

   public void recordShopBlockPlacement(Player player) {
      if (this.shopBlockFeature != null) {
         this.shopBlockFeature.recordShopBlockPlacement(player);
      }
   }
   ...
   private void giveInitialItems(Player player) {
      ...
      if (!this.storeItemFeature.hasStoreItem(player)) {
         player.getInventory().addItem(this.storeItemFeature.createStoreItem());
      }
   }
      ...
      if (sender instanceof Player player) {
         ...
         switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "remove":
               ...
               break;
            case "chat":
               player.sendMessage("§cフレンドチャットは廃止しました。DiscordのカスタムVCを使ってください。");
               break;
            default:
               player.sendMessage("§e/friend add|accept|remove");
         }
