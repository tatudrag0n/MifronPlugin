...
abstract class MifronPart11x2 extends MifronPart11x1 {
   protected void fillStatusTab(Player player, Inventory inventory) {
      ...
      inventory.setItem(10, this.named(Material.EXPERIENCE_BOTTLE, "\u00a7bMFL", List.of("\u00a77" + this.getMfl(player.getUniqueId()))));
      ...
      inventory.setItem(19, this.named(Material.IRON_SWORD, "\u00a7c\u7dcf\u30e2\u30d6\u8a0e\u4f10\u6570", List.of("\u00a77" + section.getInt("total-mob-kills", 0))));
   }
   ...
   protected void fillProgressTab(Player player, Inventory inventory, int page) {
      ...
      inventory.setItem(45, this.named(Material.WRITABLE_BOOK, "\u00a7d\u9054\u6210\u9032\u6357\u6570", List.of("\u00a77" + this.countCompletedAdvancements(player) + "/" + advancements.size())));
      inventory.setItem(49, this.named(Material.PAPER, "\u00a7e\u30da\u30fc\u30b8", List.of("\u00a77" + (safePage + 1) + "/" + (maxPage + 1))));
      if (safePage > 0) inventory.setItem(48, this.actionItem(Material.ARROW, "\u00a7f\u524d\u306e\u30da\u30fc\u30b8", List.of(), "progress_page", String.valueOf(safePage - 1)));
      if (safePage < maxPage) inventory.setItem(50, this.actionItem(Material.ARROW, "\u00a7f\u6b21\u306e\u30da\u30fc\u30b8", List.of(), "progress_page", String.valueOf(safePage + 1)));
      int slot = 9;
   ...
   protected int tabPage(String tab, String prefix) {
      ...
      return tab == null || !tab.startsWith(marker) ? 0 : this.parsePositiveInt(tab.substring(marker.length()), 0);
   }

   protected void fillReincarnationTab(Player player, Inventory inventory) {
      ConfigurationSection section = this.getPlayerSection(player.getUniqueId());
      int next = section.getInt("reincarnations", 0) + 1;
      inventory.setItem(20, this.named(Material.EXPERIENCE_BOTTLE, "\u00a7b\u8ee2\u751f\u56de\u6570", List.of("\u00a77" + section.getInt("reincarnations", 0))));
      inventory.setItem(22, this.actionItem(Material.NETHER_STAR, "\u00a7d\u8ee2\u751f\u3059\u308b", List.of("\u00a77" + this.formatNumber(this.safeMultiply(10000, next)) + "MP"), "reincarnate_now", null));
   ...
   protected void fillTitlesTab(Player player, Inventory inventory, int page) {
      ...
      int safePage = Math.max(0, Math.min(page, maxPage));
      inventory.setItem(46, this.actionItem(Material.BARRIER, "\u00a7f\u79f0\u53f7\u306a\u3057", List.of(), "clear_title", null));
      int slot = 9;
      int from = safePage * pageSize;
      for (Entry<String, TitleDefinition> entry : titles.subList(from, Math.min(titles.size(), from + pageSize))) {
         ...
         inventory.setItem(slot++, this.actionItem(entry.getValue().icon(), (unlocked ? "\u00a7a" : "\u00a78") + (unlocked ? entry.getKey() : "???"), List.of(), unlocked ? "select_title" : "locked_title", entry.getKey()));
      }
   }
