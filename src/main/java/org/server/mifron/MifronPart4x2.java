...
abstract class MifronPart4x2 extends MifronPart4x1 {
   protected void tickShelfShopActionBars() {
      ...
      for (Player player : Bukkit.getOnlinePlayers()) {
         ...
         ShelfShopOffer offer = this.readShelfShopOffer(player, player.getTargetBlockExact(5));
         ...
         player.sendActionBar(prefix.append(Component.translatable(offer.material().translationKey())
            ...
            .append(Component.text("  " + modeLabel + " / \u8ca9\u58f2:" + this.formatNumber(sellPrice) + "MP / \u8cb7\u53d6:" + this.formatNumber(buyPrice) + "MP / \u5728\u5eab:" + this.formatNumber(stock), stock > 0 ? NamedTextColor.GOLD : NamedTextColor.RED))));
      }
   }
   ...
   protected ShelfShopOffer readShelfShopOffer(Player player, Block block) {
      if (block == null || !this.isShelf(block.getType()) || !this.isShelfShop(block)) return null;
      ...
      if (configuredMaterial == null || configuredMaterial == Material.AIR || !this.isPricedShopItem(configuredMaterial)) return null;
   ...
   protected List<Material> shelfShopRandomOffers(Block block) {
      if (block == null || !this.isShelfShop(block)) return List.of();
      if ("custom".equals(this.shelfShopMode(block))) return this.customShelfShopMaterials(block);
