...
abstract class MifronPart18x1 extends MifronPart18 {
   protected void handleMerchantCommand(Player player, String[] args) {
      ...
      switch (args[1].toLowerCase(Locale.ROOT)) {
         ...
         case "clear" -> {
            ...
            for (World world : Bukkit.getWorlds()) {
               for (Entity entity : world.getEntities()) {
                  if (this.mifron().isMifronMerchant(entity)) { entity.remove(); cleared++; }
               }
            }
   ...
   protected void handleWarningCommand(CommandSender sender, String[] args) {
      ...
      int level = Math.min(4, this.mifron().parsePositiveInt(args[2], 0));
      this.mifron().getPlayerSection(target.getUniqueId()).set("warning-level", level);
      this.mifron().saveData();
      sender.sendMessage("\u00a7a" + this.mifron().safePlayerName(target) + " \u306e\u8b66\u6212\u5024\u3092 " + level + " \u306b\u3057\u307e\u3057\u305f\u3002");
      if (target.isOnline()) this.mifron().routeByWarningLevel(target.getPlayer());
   }
   ...
   protected void sendFriendRequest(Player player, OfflinePlayer target) {
      ...
      if (this.mifron().getUuidSet(player.getUniqueId(), "friends").contains(target.getUniqueId())) { player.sendMessage("\u00a7e\u3059\u3067\u306b\u30d5\u30ec\u30f3\u30c9\u3067\u3059\u3002"); return; }
      Set<UUID> requests = this.mifron().getUuidSet(target.getUniqueId(), "requests");
      ...
      player.sendMessage("\u00a7a\u30d5\u30ec\u30f3\u30c9\u7533\u8acb\u3092\u9001\u4fe1\u3057\u307e\u3057\u305f: " + this.mifron().safePlayerName(target));
   ...
   protected void acceptFriendRequest(Player player, OfflinePlayer requester) {
      Set<UUID> requests = this.mifron().getUuidSet(player.getUniqueId(), "requests");
      ...
      Set<UUID> playerFriends = this.mifron().getUuidSet(player.getUniqueId(), "friends");
      Set<UUID> requesterFriends = this.mifron().getUuidSet(requester.getUniqueId(), "friends");
      ...
      player.sendMessage("\u00a7a" + this.mifron().safePlayerName(requester) + " \u3068\u30d5\u30ec\u30f3\u30c9\u306b\u306a\u308a\u307e\u3057\u305f\u3002");
   ...
   protected void removeFriend(Player player, OfflinePlayer target) {
      Set<UUID> playerFriends = this.mifron().getUuidSet(player.getUniqueId(), "friends");
      Set<UUID> targetFriends = this.mifron().getUuidSet(target.getUniqueId(), "friends");
      ...
      player.sendMessage("\u00a7a\u30d5\u30ec\u30f3\u30c9\u3092\u89e3\u9664\u3057\u307e\u3057\u305f: " + this.mifron().safePlayerName(target));
   }
   ...
   protected void sendFriendChat(Player player, OfflinePlayer target, String message) {
      ...
      if (message.isBlank()) { player.sendMessage("\u00a7c\u672c\u6587\u304c\u672a\u5165\u529b\u3067\u3059\u3002"); return; }
      if (!this.mifron().getUuidSet(player.getUniqueId(), "friends").contains(target.getUniqueId())) { player.sendMessage("\u00a7c\u30d5\u30ec\u30f3\u30c9\u3067\u306f\u3042\u308a\u307e\u305b\u3093\u3002"); return; }
      player.sendMessage("\u00a7b[Friend -> " + this.mifron().safePlayerName(target) + "] " + message);
      if (target.isOnline()) {
         target.getPlayer().sendMessage("\u00a7b[Friend <- " + player.getName() + "] " + message);
         return;
      }
      List<String> notifications = this.mifron().getPlayerSection(target.getUniqueId()).getStringList("offline-messages");
      notifications.add(this.mifron().safePlayerName(player) + ": " + message);
      while (notifications.size() > 50) notifications.remove(0);
      this.mifron().getPlayerSection(target.getUniqueId()).set("offline-messages", notifications);
      this.mifron().saveData();
   }
}
