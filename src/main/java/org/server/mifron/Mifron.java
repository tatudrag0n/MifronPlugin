...
public final class Mifron extends JavaPlugin implements Listener, TabExecutor {
   ...
   private final QuestProposalFeature questProposalFeature = new QuestProposalFeature(this, this.proposalManager);
   ...
   void depositEmeralds(UUID uuid, int amount) {
      ...
      if (online != null && amount > 0) {
         this.recordQuestProgress(online, "mp_gained", amount);
      }
   }
   ...
   private void handleQuestCommand(CommandSender sender, String[] args) {
      if (sender instanceof Player player && this.questProposalFeature.handleCommand(player, args)) {
         return;
      }
      if (!sender.hasPermission("mifron.admin")) {
